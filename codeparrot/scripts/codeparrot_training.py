...
def setup_logging(args):
    ...
    if accelerator.is_main_process:  # we only want to setup logging once
        accelerator.init_trackers(
            ...
            config=vars(args),
            init_kwargs={"wandb": {"name": args.task_name}},
        )
...
def create_dataloaders(args):
    ...
    if ".jsonl" in args.dataset_name_train:
        train_data = load_dataset(
            "json", data_files=args.dataset_name_train, split="train", **ds_kwargs
        )
        valid_data = load_dataset(
            "json", data_files=args.dataset_name_valid, split="train", **ds_kwargs
        )
    else:
...
args.task_name = args.dataset_name_train.split("/")[-1].split("_")[0]
args.project_name = args.save_dir.split("/")[-1]
